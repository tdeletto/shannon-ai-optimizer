...
16. Fourth pass — ran that experiment under a protocol fixed before generation (primary comparison, Newcombe CI decision rule, pooling policy, all logged in advance). The nested CLI had become authenticated since pass two (the user logged in to run the first sweep), so the run was executed end-to-end in-session through the bridge.
17. The pre-committed rule fired on the "undecided" branch and blocked the revert: pooled n=25 rates identical (17/25 each). Recorded the arm that had been 5/5 dropping to 12/20 as the textbook case for why this project pre-registers decision rules. Declined to pursue the residual difference (~n=350/arm to decide a gap whose pooled estimate is zero).
18. Recorded the durable negative result honestly: stance-flip is the one unsaturated behavior on haiku and no arm — baseline included — reliably passes it; the wording rule targeting it buys nothing measurable. This moves cross-narrator consistency from "claimed by the contract" to "measured open limitation" in the README.
...
| Any sign the rewrite hurts? | **Resolved: noise.** n=20 follow-up: 14/20 vs 12/20; pooled n=25 identical at 17/25 each (diff +0.000, CI [−0.245, +0.245]) | pre-registered protocol; revert correctly blocked |
| Does anything fail unsaturated? | **Yes — stance-flip, for every arm**: baseline 16/20, v8.0 14/20, v7_3 12/20 | the explicit consistency rule buys no measurable gain |

Experiment (1) — the stance-flip pair at n=20 — **was run in the fourth pass** (120 generations via `--probes`). Verdict above: the adverse signal was noise; no contract change; the durable finding is that no arm, baseline included, reliably holds one stance across both tellings of a dispute on this model. Still open, in priority order: (1) the full sweep on a model that actually fails these probes — haiku cannot answer wording questions the ceiling hides; (2) re-run the judge with a different `--judge-model`, the 0.198 position rate having made the sonnet judge's verdicts unusable; (3) if cross-narrator consistency matters enough to pursue, it needs new probe designs and likely stronger interventions than wording — the explicit rule measurably fails to buy it.

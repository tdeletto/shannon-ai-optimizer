...
7. **A rule that cannot be shown to do anything comes out.** v8.2 removed 149 words (221 tokens) on that basis. The contract is prepended to every turn, so an unproven rule is not free — it is a permanent tax collected against a hypothesis.
8. **Count the unit that costs you, not the one that is easy to count.** The word ceilings in `eval/test_contract_files.py` are a proxy; tokens are the real budget, and the two can disagree. v8.2 looked ten words leaner than v8.0 and is one token leaner.
...
- **v8.2 and v7.4/v8.0 are equivalent on all available evidence.** The only contract difference is phrasing — tightened wording of existing rules and the first-person shift — which was never A/B'd on its own, and which costs **one token less** (941 vs 942), not the ten words a naive count suggests. Nothing measured separates them. Preferring v8.2 is a judgment about prose; preferring v8.0 because it is the last text with a full live run behind it is equally defensible.
- **Very short, one-off chats:** measured against the tokenizer, the instructions add **436 tokens** (`daily`) or **941** (full contract); on a single trivial question the overhead can exceed the savings. The benefit compounds over multi-turn sessions and longer outputs.
